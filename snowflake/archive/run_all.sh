#!/usr/bin/env bash
# Executes all Snowflake SQL scripts in order against the <YOUR_CONNECTION> account.
# Usage:
#   ./run_all.sh                           # core setup only (synthetic data mode)
#   ./run_all.sh <YOUR_CONNECTION> --marketplace      # also run Marketplace sources SQL
# Requires SnowSQL CLI installed and configured.

set -euo pipefail

CONNECTION="${1:-<YOUR_CONNECTION>}"
...
echo "     (from: snowsql -c <YOUR_CONNECTION> -q 'DESC INTEGRATION AWS_S3_CRYPTO;')"
...
echo "  6. snowsql -c <YOUR_CONNECTION> -f sql/demo/demo_seed.sql   (step 8 onwards)"
