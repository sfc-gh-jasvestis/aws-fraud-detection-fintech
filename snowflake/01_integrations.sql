...
-- Allows Snowflake to access s3://crypto-surveillance-raw-__AWS_ACCOUNT_ID__
CREATE STORAGE INTEGRATION IF NOT EXISTS AWS_S3_CRYPTO
    TYPE = EXTERNAL_STAGE
    STORAGE_PROVIDER = 'S3'
    ENABLED = TRUE
    STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::__AWS_ACCOUNT_ID__:role/crypto-surveillance-snowflake-s3-role'
    STORAGE_ALLOWED_LOCATIONS = ('s3://crypto-surveillance-raw-__AWS_ACCOUNT_ID__/')
...
CREATE STAGE IF NOT EXISTS CRYPTO_SURVEILLANCE.RAW.RAW_S3_STAGE
    URL = 's3://crypto-surveillance-raw-__AWS_ACCOUNT_ID__/'
