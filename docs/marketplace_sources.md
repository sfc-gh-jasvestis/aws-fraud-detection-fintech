...
Marketplace listing that is auto-provisioned on most Snowflake accounts (including `<YOUR_CONNECTION>`).
...
> **Note**: Availability varies by account tier and region. For <YOUR_CONNECTION> (internal), try Cybersyn first — it is typically pre-approved for Snowflake internal accounts.
...
3. Select your Snowflake account (`<YOUR_CONNECTION>`) as the consumer
